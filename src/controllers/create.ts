...
import { BadRequestError, IMessageDocument, uploas } from '@mjmakenzi/jobber-shared';
...
const message = async (req: Request, res: Response): Promise<void> => {
  ...
  if (file) {
    result = (
      req.body.fileType === 'zip' ? await uploas(file, `${randomCharacters}.zip`) : await uploas(file)
